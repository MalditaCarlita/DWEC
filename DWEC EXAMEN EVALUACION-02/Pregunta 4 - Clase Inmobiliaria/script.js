...
class Inmobiliaria extends Edificio{
    ...
    borrarEdificios(edificio){
        this.edificios=delete(edificios[edificio]);
        this.numEdificios++;
        salida.innerText +="Se ha borrado el edificio";
    }
    imprimirEdificios(){
        for(var i=0; i<this.edificios.length;i++){
            salida.innerText+= "Listado de propiedades de la inmobiliaria " +this.nombre +":\n";
            salida.innerText += "- Edificio "+(i+1) +": en la calle " +this.edificios[i].calle +", nº:" +this.edificios[i].numero +" y código postal " +this.edificios[i].codigo +".\n";
        }
    }
}
